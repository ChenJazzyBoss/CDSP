...
def train(args, use_modal, local_rank):
    ...
    if 'None' not in args.load_ckpt_name:
        ...
        try:
            torch.cuda.set_rng_state(checkpoint['cuda_rng_state'])
            print("✅ CUDA RNG state restored successfully")
        except RuntimeError as e:
            if "RNG state is wrong size" in str(e):
                print(f"⚠️  Warning: Skipping CUDA RNG state restoration due to GPU count mismatch")
                print(f"   This may slightly affect randomness but won't impact model performance")
            else:
                raise e
        is_early_stop = True
        model.freeze()  # 冻结模型参数
